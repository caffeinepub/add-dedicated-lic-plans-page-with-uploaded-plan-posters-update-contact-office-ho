...
interface MaturityCalculatorFormProps {
  onCalculate: (inputs: { premiumAmount: number; term: number; age: number }) => void;
}

export function MaturityCalculatorForm({ onCalculate }: MaturityCalculatorFormProps) {
  const { t } = useLanguage();
    ...
    <Card className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Button type="submit" size="lg" className="w-full md:w-auto">
          {t({ mr: 'परिपक्वता मोजा', hi: 'परिपक्वता की गणना करें', en: 'Calculate Maturity' })}
        </Button>
      </form>
    </Card>
  );
